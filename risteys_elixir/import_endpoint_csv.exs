...
|> Stream.map(fn row ->
  reason =
    cond do
      row.reason_non_core == "" -> nil
      row.reason_non_core =~ "EXALLC priorities" -> "exallc_priority"
      row.reason_non_core =~ "Correlated endpoint" -> "correlated"
      true -> "other"
    end
...
# Prefetch all endpoint IDs
endpoints_ids = FGEndpoint.list_endpoints_ids()

Enum.each(map_corr_selected, fn {correlated, selected} ->
