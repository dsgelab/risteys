...
import {
  ...
  uniq,
  zipWith,
} from "../vendor/lodash-full.v4.17.15.js";
...
config.xTickLabels.yBaseline =
  config.canvas.height - config.xTickLabels.marginBot; // px
...
let scaleX = (data) => {
  ...
  return d3
    .scaleLinear()
    .domain(
      d3.extent(flatMap(cdata, (d) => [d.interval.left, d.interval.right])),
    )
...
function capbinPosDim(data) {
  return map(data, (datum) => {
    ...
    const height =
      config.vbinSurface.y2 -
      config.vbinSurface.y1 + // vbinSurface height
...
function xTicksAndLabels(data) {
  let values = flatMap(data, (datum) => [
    datum.interval.left,
    datum.interval.right,
  ]);
  values = uniq(values);
...
// Y ticks
let yAxis = (data) => {
  const axis = d3.axisLeft(scaleY(data)).ticks(10, "s"); // shortens tick labels, e.g. 6000 -> 6k
  return (g) => {
    g.attr("transform", `translate(${config.margin.left}, 0)`).call(axis);
  };
};
...
function plot(selector, data, xAxisLabel, yAxisLabel, plot_color) {
  ...
  // Bin groups
  const groups = svg.selectAll("g").data(pdata).enter().append("g");
  ...
  groups
    ...
    .on("touchstart mouseenter", (_event, d) => {
      d3.select(d.selector).style("display", "block");
    })
    .on("touchend mouseleave", (_event, d) => {
      d3.select(d.selector).style("display", "none");
    });
  ...
  svg
    ...
    .attr(
      "transform",
      `translate(${config.yAxisLabel.x}, ${config.yAxisLabel.y}) rotate(-90)`,
    )
    .text(yAxisLabel);
